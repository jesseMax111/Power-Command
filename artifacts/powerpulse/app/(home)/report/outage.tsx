...
import { View, Text, TextInput, Pressable, StyleSheet, ScrollView, ActivityIndicator, Platform, Alert } from "react-native";
...
export default function ReportOutageScreen() {
  ...
  const getLocation = async () => {
    ...
    try {
      ...
      if (status !== "granted") {
        Alert.alert("Permission denied", "Location permission is needed to report your exact location.");
        return;
      }
  ...
  const handleSubmit = () => {
    if (!location) {
      Alert.alert("Location needed", "Please wait for your location or enter it manually.");
      return;
    }
    ...
    createReport(
      {
        data: {
          ...
          userName: user?.fullName || user?.emailAddresses?.[0]?.emailAddress || undefined,
        },
      },
      {
        onSuccess: () => {
  ...
  return (
    <View style={[styles.screen, { backgroundColor: c.background }]}>
      <View style={[styles.header, { paddingTop: insets.top + 16 }]}>
        ...
        <Text style={[styles.headerTitle, { color: c.foreground }]}>Report Outage</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.iconRow}>
          <View style={[styles.iconBg, { backgroundColor: c.powerOff + "20" }]}>
            <Ionicons name="flash-off" size={32} color={c.powerOff} />
          </View>
          <Text style={[styles.subtitle, { color: c.mutedForeground }]}>Alert your community about a power outage</Text>
        </View>
        ...
        <View style={[styles.locationCard, { backgroundColor: c.card, borderColor: c.border }]}>
          <View style={styles.locationRow}>
            ...
            {locLoading ? (
              <ActivityIndicator size="small" color={c.primary} style={{ flex: 1 }} />
            ) : location ? (
              <Text style={[styles.locationText, { color: c.foreground }]}>
                {location.latitude.toFixed(5)}, {location.longitude.toFixed(5)}
              </Text>
            ) : (
              <Text style={[styles.locationText, { color: c.mutedForeground }]}>Location not detected</Text>
            )}
        ...
        <TextInput
          style={[styles.input, styles.notesInput, { backgroundColor: c.card, borderColor: c.border, color: c.foreground }]}
          value={description}
        ...
        <Pressable
          style={({ pressed }) => [styles.submitBtn, { backgroundColor: c.powerOff, opacity: pressed || isPending ? 0.85 : 1 }]}
        ...
        >
          {isPending ? (
            ...
            <>
              <Ionicons name="flash-off" size={20} color={c.foreground} />
              <Text style={[styles.submitText, { color: c.foreground }]}>Report Power Out</Text>
            </>
          )}
        </Pressable>
      </ScrollView>
